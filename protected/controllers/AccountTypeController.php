...
class AccountTypeController extends CController
{
	...
	public function accessRules()
	{
		return array(
			array('allow', 
				...
				'expression'=>'Yii::app()->user->getState(\'allowAccountTypes\')',
			),
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['American']) || isset($_POST['Swedish']))
		{
			...
			if (isset($_POST['Swedish'])) $filename="DefaultAccountTypes.SE.xml"; 
		...
		if(isset($_POST['AccountType']))
		{
			...
			if($model->save()){
				...
				if ($modelBeforeChange!=$stringModel)
					ChangeLog::addLog('UPDATE','AccountType','BEFORE<br />' . $modelBeforeChange . '<br />AFTER<br />' . $stringModel);
				$this->redirect(array('admin','id'=>$model->id));
			}
		}
	...
	public function actionAdmin()
	{
		...
		$pages=new CPagination(AccountType::model()->count($criteria));
		$pages->pageSize=Yii::app()->user->getState('NumberRecordsPerPage');
		$pages->applyLimit($criteria);

		$sort=new CSort('AccountType');
	...
	protected function processAdminCommand()
	{
		...
		{
			$deleteAccountType=$this->loadAccountType($_POST['id']);
			ChangeLog::addLog('DELETE','AccountType',$deleteAccountType->toString());
